...
public class BitcoVerifier2 extends TestBase {
	...
	public String[][] getRegistrationDataFromExcel() throws Exception {
		...
		String str[][] = AppLibrary
				.readExcel(appLibrary.getConfiguration().getSourcePath() + File.separator + "bitco.xls", 1);
		return str;
	}
	...
	public void setup() throws Exception {
		try {
			...
			writer = new PrintWriter(appLibrary.getConfiguration().getResultPath() + File.separator + "bitco2.txt",
					"UTF-8");
			writer.close();
	...
	public void jokerOne(String ID, String pathToFront, String pathToBack, String proxy, String ExecutionIndicator) {
		...
		if (ExecutionIndicator.equalsIgnoreCase("Yes")) {
			...
			try {
				File file = new File(appLibrary.getConfiguration().getResultPath() + File.separator + "bitco2.txt");

...
public class BitcoVerifier4 extends TestBase {
	...
	public String[][] getRegistrationDataFromExcel() throws Exception {
		...
		String str[][] = AppLibrary
				.readExcel(appLibrary.getConfiguration().getSourcePath() + File.separator + "bitco.xls", 3);
		return str;
	}
	...
	public void setup() throws Exception {
		try {
			...
			writer = new PrintWriter(appLibrary.getConfiguration().getResultPath() + File.separator + "bitco4.txt",
					"UTF-8");
			writer.close();
			String socks = appLibrary.getConfiguration().getSocksProxy();
			String[] socks_addr = socks.split(":");
			String IP = socks_addr[0];
			String port = socks_addr[1];
			
			
			ChromeOptions options = new ChromeOptions();
			options.addArguments("--lang=en-us");
			options.addArguments("--proxy-server=socks5://" + IP + ":" + port);
	...
	public void jokerOne(String ID, String pathToFront, String pathToBack, String proxy, String ExecutionIndicator) {
		...
		if (ExecutionIndicator.equalsIgnoreCase("Yes")) {
			...
			try {
				...
				String apiKey = "b0d420362f1aa63d3ffe43ba44020ef47c";
				String googleKey = "6Le205uoSAAAAAH3LKzssY-LHQOMu6eBag0yqlA6O";
				...
				SyncElement(driver, "(//div[@class='id-box'])[3]", 20).click();

				// Choose File Upload vs. Webcam
				SyncElement(driver, "//button[@class='btn btn-large btn-upload']", 20).click();

				// Upload the front
				SyncElement(driver, "//input[@class='file-input']", 20)
						.sendKeys(appLibrary.getConfiguration().getSourcePath() + File.separator + pathToFront);

				// Continue
				SyncElement(driver, "//button[@class='btn btn-primary']", 20).click();
			...
			try {
				File file = new File(appLibrary.getConfiguration().getResultPath() + File.separator + "bitco4.txt");
